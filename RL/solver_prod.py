...
################################################################## 
S_mean = [432, 14, 20, 20]
def Solver_Pdn(theta, nmrec=115, S_mean=S_mean): 
    """
    ...
    Model.AddVarLocal( typ='State', varid='H', prn=r'$H_k$',\
               desc="Accumulated weight of all harvested fruits.", units= g, val=1000.0) # peso total de los pepinos
    Model.AddVarLocal( typ='State', varid='NF', prn=r'$N_k$',\
               desc="Accumulated  number of fruits harvested", units= n_f, val=7.0)
    Model.AddVarLocal( typ='State', varid='h', prn=r'$h_k$',\
               desc="Weight of all harvested fruits.", units= g, val=1000.0)
    Model.AddVarLocal( typ='State', varid='n', prn=r'$n_k$',\
               desc="Total  number of fruits harvested", units= n_f, val=7.0)
    ### Run:
