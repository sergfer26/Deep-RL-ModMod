...
def Sol_Pdn(theta, nmrec=115): 
    ...
    La función retorna resultados en este orden: ( m, n, h, NF, H ).
    """
    ...
    class Q_rhs(StateRHS):
        ...
        def __init__( self ):
            ...
            self.AddVar( typ='State', varid='T', prn=r'$T$',\
                desc="Greenhouse air temperature", units= C , val=20, rec=nrec)
    ...
    class Plant(Module):
        ...
        def Advance( self, t1):
            ...
            for i in range( 1, steps):
                ...
                hk = 380*nk + norm.rvs( scale=sqrt(nk)*theta[3] ) # Peso de frutos considerando aleatoriedad
                self.V_Set( 'n_k', nk)
                self.V_Set( 'h_k', hk)
    ...
    class GreenHouse(Director):
        def Scheduler( self, t1, sch):
            ...
            for p, plant in enumerate(Model.PlantList):
                ...
                t_m_k += Model.Modules[plant].Modules['Plant'].V('m_k')
    ...
    Model.AddVarLocal( typ='State', varid='H', prn=r'$H_k$',\
               desc="Accumulated weight of all harvested fruits.", units= g, val=0.0)
    Model.AddVarLocal( typ='State', varid='NF', prn=r'$N_k$',\
               desc="Accumulated  number of fruits harvested", units= n_f, val=0.0)
    Model.AddVarLocal( typ='State', varid='h', prn=r'$h_k$',\
               desc="Weight of all harvested fruits.", units= g, val=0.0)
    Model.AddVarLocal( typ='State', varid='n', prn=r'$n_k$',\
               desc="Total  number of fruits harvested", units= n_f, val=0.0)
    Model.AddVarLocal( typ='State', varid='m', prn=r'$m_k$',\
    ...
    return Model.OutVar('m'), Model.OutVar('n'), Model.OutVar('h'), NF, H # m -> determinista frutos cosechaodos


##################################################################
######### Lectura datos de producción Holanda ####################
##################################################################
def OutData_Pdn(nmrec=114): # Se tienen datos para 115 días, pero hay problemas para leer los datos del último día. Estos deben agregarse al final a mano
    """
    Retorna los datos de Holanda, cada día
    desde cero hasta el día nmrec.
    Los datos se entregan en este orden: ( NF, H)
    """
    Dir1 = Director( t0=0.0, time_unit=d, Vars={}, Modules={} )
    nrec = nmrec
    ### Add variables manually ...
    ### these typically will be first defined by other modules, RHss
    ### and already available in the Director
    Dir1.AddVar( typ='State', varid='H_H', prn=r'$H_H$',\
                desc="Peso total frutos", units= g, val=0, rec=nrec)
    Dir1.AddVar( typ='State', varid='NF_H', prn=r'$NF_H$',\
                desc="Número total de frutos", units= n_f, val=0, rec=nrec)
    ### Add an instance of Module 1:
    Dir1.AddModule( 'Climate', \
        ReadModule( "Read_Pdn.xls", t_conv_shift=0.0, t_conv=1.  )) # The data base time is days
    Dir1.sch = [ 'Climate' ]       
    Dir1.Run( Dt=1, n=nrec, sch= Dir1.sch)
    NF_H = Dir1.OutVar('NF_H')
    H_H = Dir1.OutVar('H_H')
    H_H = append(H_H, 33818.77049) # se agrega los últimos valores a mano
    NF_H = append(NF_H, 88.729508197) # pues con 115 hay problemas
    ## Correcciones
    NF_H[55:] *= 1.2
    H_H[55:] *= 1.2
...
theta_nm = array([0.7, 3.3, 0.25, 1]) # theta nominal
Res = Sol_Pdn(theta_nm)
NF = Res[3] 
H = Res[4]
Data = OutData_Pdn()
NF_H = Data[0]
